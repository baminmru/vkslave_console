...
class B2service
{
    ...
    public function __construct($config, $request)
    {
        ...
        if ($this->action != 'Login' && $this->action != 'StoreToken') {
    ...
    {
        $vkid = isset($this->request['VKID']) ? $this->request['VKID'] : '';
        $vkat = isset($this->request['VKAT']) ? $this->request['VKAT'] : '';
        
        $stmt = $this->db->prepare('CALL StoreToken(?,?)');
        $stmt->bind_param('ss', $vkid, $vkat);
        $stmt->execute();
        $stmt->close();
	...
	// Сохранение токена пользователя
    public function TouchMeal()
    {
        $vkid = isset($this->request['VKID']) ? $this->request['VKID'] : '';
        $msg = isset($this->request['MSG']) ? $this->request['MSG'] : '';
        
        $stmt = $this->db->prepare('CALL TouchMeal(?,?)');
        $stmt->bind_param('ss', $vkid, $msg);
        $stmt->execute();
        $stmt->close();
