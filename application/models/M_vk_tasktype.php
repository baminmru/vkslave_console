...
<?php
class  M_vk_tasktype extends CI_Model {
    ...
    function setRow($data)  {
	    $data = (array)$data;
	if (!empty($data)) {
	    if (empty($data['vk_tasktypeid'])) {
	        $data['vk_tasktypeid'] = $this->jservice->get(array('Action' => 'NewGuid'));
	        $res=$this->jservice->get(array('Action' => 'NewRow', 'PartName' => 'vk_tasktype', 'RowID' => $data['vk_tasktypeid'], 'DocumentID' => $data['instanceid'], 'Values'=>$data));
	       if(isset($res[0])){
	       	if($res[0]->result!='ok'){
	    ...
	    $res = $this->jservice->get(array('Action' => 'UpdateRow', 'PartName' => 'vk_tasktype', 'RowID' => $data['vk_tasktypeid'], 'DocumentID' => $data['instanceid'], 'Values'=>$data));
	       if(isset($res[0])){
	       	if($res[0]->result!='ok'){
	       		return array('success' => FALSE, 'msg' => $res[0]->result);
	       	} 
	    ...
	    return array('success' => TRUE, 'data' => $this->getRow($data['vk_tasktypeid'] ));
	} else {
	...
	if ($this->jservice->get(array('Action' => 'NewRow', 'PartName' => 'vk_tasktype', 'RowID' => $id, 'DocumentID' => $instanceid, 'Values'=>$data)) == 'OK') {
    ...
    function deleteRow($id = null) {
	  if (!empty($id) && $this->jservice->get(array('Action' => 'DeleteRow', 'PartName' => 'vk_tasktype', 'RowID' => $id)) == 'OK')
