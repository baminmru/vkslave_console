...
function DefineInterface_vk_brkinfo_main(id,mystore,selection){
...
var p1_valid=false;
     function onSave(close_after_save,callaftersave){
        ...
        if (form.isValid()) {
            ...
            Ext.Ajax.request({
                url: rootURL+'index.php/c_vk_brkinfo/setRow',
                method:  'POST',
                params: { 
                    instanceid: p1.instanceid
                    ,vk_brkinfoid: active.get('vk_brkinfoid')
	            ...
	            if(res.success==false){
                    ...
                    if(active.get('vk_brkinfoid')==''){
               			active.set('vk_brkinfoid',res.data['vk_brkinfoid']);
                    }
        		   /* Ext.MessageBox.show({
                        title:  'Подтверждение',
                   ...
                   if(selection){
                     Ext.Ajax.request({
                        url: rootURL+'index.php/c_v_autovk_brkinfo/getRows?&filter=[{"property":"vk_brkinfoid","value":"'+ active.get('vk_brkinfoid') + '"}]',
                        method:     'GET',
                        success: function(response){
...
function DefineForms_vk_brkinfo_main(){


Ext.define('Form_vk_brkinfomain', {
extend:  'Ext.form.Panel',
alias: 'widget.f_vk_brkinfomain',
...
allowBlank:true
    ...
    onReset: function(){
        if(this.activeRecord.get('vk_brkinfoid')==''){
...
Ext.define('EditWindow_vk_brkinfomain', {
