...
var groupingFeature_autovk_prj = Ext.create('Ext.grid.feature.Grouping',{
groupByText:  'Группировать по этому полю',
showGroupsText:  'Показать группировку'
});
var interval_autovk_prj;
Ext.define('grid_autovk_prj', {
    extend:  'Ext.grid.Panel',
    alias: 'widget.g_v_autovk_prj',
    requires: [
        'Ext.grid.*',
        'Ext.form.field.Text',
    ...
    initComponent: function(){
        Ext.apply(this, {
        frame: false,
        store: store_v_autovk_prj,
        features: [groupingFeature_autovk_prj],
        ...
        columns: [
            {text: "Название", width:400, dataIndex: 'vk_prj_title', sortable: true}
            ,
            {text: "Описание", width:400, dataIndex: 'vk_prj_info', sortable: true}
        ]
        ,
...
listeners: {render: function(e) {Ext.QuickTips.register({  target: e.getEl(), text: 'Описание'});}}
    ...
    listeners: {
        ...
        ,
        	added:function(){
        			//interval_autovk_prj= setInterval(function() {  
        			//	store_v_autovk_prj.load();
        			//}, 60000);
        		}
        	,
        	destroy:function(){
        		//clearInterval(interval_autovk_prj);
        }
    },
    onDeleteConfirm:function(selection){
      if (selection) {
        Ext.Ajax.request({
            url:    rootURL+'index.php/c_v_autovk_prj/deleteRow',
            method:  'POST',
    		params: { 
    ...
    onDeleteClick: function(){
      ...
      if (selection) {
   	    if(CheckOperation('VKPRJ.edit')!=0 && OTAllowDelete('VKPRJ')){
        Ext.Msg.show({
            title:  'Удалить?',
    ...
    onAddClick: function(){
   	    if(CheckOperation('VKPRJ.edit')!=0 && OTAllowAdd('VKPRJ')){
            Ext.Ajax.request({
                url: rootURL+'index.php/c_v_autovk_prj/newRow',
                method:  'POST',
                params: { 
                },
                ...
                edit.prefix='c_v_autovk_prj';
                edit.setTitle('Создание документа:Рекламный проект') ;
                var p=eval('VKPRJ_Panel_'+OTAddMode('VKPRJ')+'( res.data, false,null )') ;
                edit.add(p);
                edit.show();
                }
            });
    ...
    onEditClick: function(){
        var selection = this.getView().getSelectionModel().getSelection()[0];
        if (selection) {
   	    if(CheckOperation('VKPRJ.edit')!=0 ){
                var edit = Ext.create('iu.windowObjects');
                edit.prefix='c_v_autovk_prj';
                edit.setTitle('Редактирование документа: Рекламный проект') ;
            var p=eval('VKPRJ_Panel_'+OTEditMode('VKPRJ')+'( selection.get(\'instanceid\'), false, selection )') ;
            edit.add(p);
            edit.show();
        }else{
        		Ext.MessageBox.show({
...
function VKPRJ_Jrnl(){ 
  ...
  var VKPRJ= Ext.create('Ext.form.Panel', {
       closable: true,
       id:     'vkprj_jrnl',
      ...
      fieldDefaults: {
        ...
        items: [{
            itemId:'gr_autovk_prj',
            xtype:'g_v_autovk_prj',
            stateful: stateFulSystem,
            stateId:'j_v_autovk_prj',
            layout: 'fit',
            flex: 1,
            store: store_v_autovk_prj
    }] // tabpanel
